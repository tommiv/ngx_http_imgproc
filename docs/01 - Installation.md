...
You will need **git**. If you don't have git, just install it with a package manager
...
FreeImage library provides support for huge list of input/output image formats, including WEBP, TIFF and GIF (with **animation support**). If you don't need this, just comment out _IMP\_FEATURE\_ADVANCED\_IO_ flag at the top of required.h file and skip this section. Without FreeImage, IMP is restricted to OpenCV codec, and therefore has support only for JPEG and PNG.

#### Through package

Before you go, there is a **huge caveat**: it probably will install 3.15.x version of library. WEBP therefore will be unavailable. (Official site also says: _It (v3.16) also provides better support for RAW, JPEG-2000, BMP, SGI, ICO, PSD, TIFF, TGA, GIF formats_).

If you ok with this, just run

    sudo apt-get install libfreeimage-dev

or use a package provided by your OS. Otherwise go to the next header.

#### From source

1. Go to http://freeimage.sourceforge.net/download.html and copy the link to the source. Let's say it is http://downloads.sourceforge.net/freeimage/FreeImage3160.zip
...
**[Optional].** You can download `pcre`, `openssh` and/or `zlib`. It is used for some nginx features, but it's not necessary if you build IMP-enabled nginx as a separatee service. As for me, I downloaded `pcre`.
...
I assume you still in `/var/opt/src` and you have all the needed sources here: nginx, optional pcre|libssh|zlib and IMP.
...
You can just copy/paste it if `/var/opt/bin/nginx-imp` binary path is ok for you. If you download any additional libs like zlib, you can add them to configure the command, as described [here](http://wiki.nginx.org/InstallOptions).

Wait until the configuration is finished. If you don't see an "error" word, everything is ok.
    
Now is your chance. Type

    make
    
If you're lucky enough, you'll see a compiler message that everything is complete. Check for it:
...
If you see `libdc1394 error: Failed to initialize libdc1394` – don't worry too much. This is not an error actually, it's a warning. I wasn't able to found an easy fix for this, but it has no any consequences, it's just annoying.
...
and start to configure the service.
...
If you used the same binary path as me, just put `/docs/nginx-imp` script to `/etc/init.d/`. Otherwise open it first and replace DAEMON and PIDFILE values on lines 15-16 with your own ones.
...
Use signals to stop/restart the service
...
2. Create build.sh file with this content:
    ...
    **Important**: building openCV, FreeImage and nginx itself inside docker is too complex, slow and it also leads to too big docker image, so we just copy pre-built libs and the executable to the image. However, the code above assumes that openCV is 2.4.x branch and FreeImage is 3.x branch and both libs are built from sources as it's described in this manual. It's up to you to check if this libs really exist in its places. Note you can use `readelf -d nginx | grep NEEDED` to get list of dynamic libs required by nginx-imp executable.
...
5. If the build process ends well, you need to edit /etc/nginx-imp/conf/nginx.conf and put `daemon off` directive into it. It's really important since docker will close container if nginx daemonizes itself.
